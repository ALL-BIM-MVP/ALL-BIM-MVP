...
const IFCViewer = forwardRef<IFCViewerHandle, IFCViewerProps>(({ fileBuffer, fragmentsBuffer = null, projectId, onFileUploaded, viewCubeRightOffset = 0, viewCubeVisible = true, isActive = true, onViewElementInMetrados }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    isolateElementsByIds: (expressIds: number[], globalIds?: (string | null)[]) => {
      ...
      return usingFragmentsSelection ? isolateFragmentsElementsByIds(expressIds, globalIds) : isolateElementsByIds(expressIds);
    },
    selectGroupInViewer: (expressIds: number[], globalIds?: (string | null)[]) => {
      setSearchResetKey((k) => k + 1);
      return usingFragmentsSelection ? selectFragmentsGroupInViewer(expressIds, globalIds) : selectGroupInViewer(expressIds);
    },
